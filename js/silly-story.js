...
function result() {
  ...
  newStory = newStory.replace(/:insertX:/g, xItem); // "/g" is used to make sure all values of ":insertX:" are replaced rather than just one 
  newStory = newStory.replace(":insertY:", yItem); // not needed here with only one instance of ":insertY:"
  newStory = newStory.replace(":insertZ:", zItem); // not needed here with only one instance of ":insertz:"
  ...
  if (document.getElementById("metric").checked) {
   // STEP 11a: Create a variable called weight and convert the 300lbs to kgs (1lb = 0.453592kg)
    ...
    newStory = newStory.replace("300 pounds", `${weight} kg`); // `${}` instead of + weight + 
